...
async def guard_node(state): return state  # unchanged, same as before

async def clarifier_node(state): return state  # unchanged

async def planner_node(state):
    msg = state["messages"][-1]["content"]
    plan = _default_plan(msg)

    # Try LLM for customization
    try:
...
只输出JSON:"""
        ...
        if s != -1 and e != -1:
            ...
            if len(llm_plan) >= 3: plan = llm_plan  # Only use if valid
    except: pass
...
def route_guard(s): return "end" if s.get("guard_blocked") else "clarifier"
def route_clarifier(s): return "end" if s.get("need_clarify") else "planner"
...
def build_graph():
    ...
    g.add_node("guard",guard_node); g.add_node("clarifier",clarifier_node)
    g.add_node("planner",planner_node); g.add_node("executor",executor_node)
    g.add_node("aggregator",aggregator_node)
    g.set_entry_point("guard")
    g.add_conditional_edges("guard",route_guard,{"end":END,"clarifier":"clarifier"})
    g.add_conditional_edges("clarifier",route_clarifier,{"end":END,"planner":"planner"})
